...
#ifndef SL_BYTE_SIZE
#define SL_BYTE_SIZE 8 // byte size in bits
#endif
#define SL_MEGABYTE (1024 * 1024)

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `SlikeAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, SlikeAnimatedImageDataSizeCategory) {
    SlikeAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    SlikeAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    SlikeAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    SlikeAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, SlikeAnimatedImageFrameCacheSize) {
    SlikeAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    SlikeAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    SlikeAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    SlikeAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


@interface SlikeAnimatedImage ()
{
...
// The actual type of the object is `SLWeakProxy`. Lazily instantiated since it is not typically needed.
@property (nonatomic, strong, readonly) SlikeAnimatedImage *weakProxy;

@end


@implementation SlikeAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > SlikeAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > SlikeAnimatedImageFrameCacheSizeNoLimit) {
...
- (SlikeAnimatedImage *)weakProxy
{
    if (!_weakProxy) {
        _weakProxy = (id)[SLWeakProxy weakProxyForObject:self];
    }
...
- (id)init
{
    NSLog(@"Error: Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `SlikeAnimatedImage`.");
    return nil;
}
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data
{
    ...
    if (self) {
        ...
        CGFloat animatedImageDataSize = CGImageGetBytesPerRow(self.posterImage.CGImage) * self.size.height * self.frameCount / SL_MEGABYTE;
        if (animatedImageDataSize <= SlikeAnimatedImageDataSizeCategoryAll) {
            _frameCacheSizeOptimal = self.frameCount;
        } else if (animatedImageDataSize <= SlikeAnimatedImageDataSizeCategoryDefault) {
            ...
            _frameCacheSizeOptimal = SlikeAnimatedImageFrameCacheSizeDefault;
        } else {
            // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
            _frameCacheSizeOptimal = SlikeAnimatedImageFrameCacheSizeLowMemory;
        }
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[SlikeAnimatedImage class]]) {
        SlikeAnimatedImage *animatedImage = (SlikeAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        NSLog(@"Error: `image` isn't of expected types `UIImage` or `SlikeAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = SlikeAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    SlikeAnimatedImage *strongSelf = self;
    
    strongSelf.memoryWarningCount++;
    
    // If we were about to grow larger, but got rapped on our knuckles by the system again, cancel.
    [NSObject cancelPreviousPerformRequestsWithTarget:strongSelf.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(SlikeAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    NSLog(@"Verbose: Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)SlikeAnimatedImageFrameCacheSizeLowMemory, (unsigned long)strongSelf.frameCacheSizeMaxInternal, (unsigned long)strongSelf.memoryWarningCount, strongSelf);
    strongSelf.frameCacheSizeMaxInternal = SlikeAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((strongSelf.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [strongSelf.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(SlikeAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
    ...
    size_t bytesPerPixel = (bitsPerPixel / SL_BYTE_SIZE);
...
#pragma mark - SLWeakProxy

@interface SLWeakProxy ()
...
{
    SLWeakProxy *weakProxy = [SLWeakProxy alloc];
